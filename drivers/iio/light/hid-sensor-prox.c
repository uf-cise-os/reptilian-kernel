...
struct prox_state {
	...
	struct hid_sensor_hub_attribute_info prox_human;
	u32 human_presence;
};
	...
	*val2 = 0;
	...
	case IIO_CHAN_INFO_RAW:
		...
		case  CHANNEL_SCAN_INDEX_PRESENCE:
			report_id = prox_state->prox_human.report_id;
			address =
	...
	case IIO_CHAN_INFO_SCALE:
		*val = prox_state->prox_human.units;
		ret_type = IIO_VAL_INT;
		break;
	case IIO_CHAN_INFO_OFFSET:
		*val = hid_sensor_convert_exponent(
				prox_state->prox_human.unit_expo);
		ret_type = IIO_VAL_INT;
		break;
...
static int prox_write_raw_get_fmt(struct iio_dev *indio_dev,
			       struct iio_chan_spec const *chan,
			       long mask)
{
...
static const struct iio_info prox_info = {
	...
	.write_raw_get_fmt = &prox_write_raw_get_fmt,
};
...
static int hid_prox_probe(struct platform_device *pdev)
{
	...
	indio_dev = iio_device_alloc(sizeof(struct prox_state));
	if (indio_dev == NULL) {
		ret = -ENOMEM;
		goto error_ret;
	}
...
MODULE_DESCRIPTION("HID Sensor Prox");
