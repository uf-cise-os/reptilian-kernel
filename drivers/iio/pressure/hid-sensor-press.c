...
static int press_write_raw_get_fmt(struct iio_dev *indio_dev,
			       struct iio_chan_spec const *chan,
			       long mask)
{
...
static const struct iio_info press_info = {
	...
	.write_raw_get_fmt = &press_write_raw_get_fmt,
};
...
static int hid_press_probe(struct platform_device *pdev)
{
	...
	indio_dev = iio_device_alloc(sizeof(struct press_state));
	if (indio_dev == NULL) {
		ret = -ENOMEM;
		goto error_ret;
	}
